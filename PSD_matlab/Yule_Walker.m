...
function f0s = Yule_Walker(s, fs, N, L, order, nfft)
    f0s = []; segments = (1:N:L-N); 
    for n_start = segments
        x = s(n_start : n_start+N-1);  
        [PSD, f_fft] = pyulear(x,order, nfft, fs);
        [~, idx] = max(PSD); f0 = f_fft(idx);
        f0s(end+1) = f0;
        
        if n_start == segments(81)
            orders = (4:4:16);
            figure;  set(gcf, 'Position', [1300, 600, 600, 1200]); set(gca, 'DefaultTextFontSize', 14, 'FontName', 'times')
            ...
            for order = orders
                ...
                title(sprintf('Yule-Walker (Order: %d)', order), 'fontsize', 18); 
